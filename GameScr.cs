...
public class GameScr : mScreen, IChatable
{
	...
	public bool isAttack()
	{
		...
		if (Char.myCharz().mobFocus != null)
		{
			...
			if (num > Char.myCharz().myskill.dx || num2 > Char.myCharz().myskill.dy)
			{
				...
				int num3 = (Char.myCharz().myskill.dx - ((!flag) ? 20 : 50)) * ((Char.myCharz().cx > Char.myCharz().mobFocus.getX()) ? 1 : (-1));
				if (num <= Char.myCharz().myskill.dx)
				{
					num3 = 0;
				}
				Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().mobFocus.getX() + num3, Char.myCharz().mobFocus.getY());
			...
			{
				int num5 = ((Char.myCharz().cx > Char.myCharz().mobFocus.getX()) ? 1 : (-1));
				if ((TileMap.tileTypeAtPixel(Char.myCharz().mobFocus.getX() + num4 * num5, Char.myCharz().cy + 3) & 2) != 2)
				{
					flag3 = true;
				}
			}
			if (num <= num4 && !flag3)
			{
				if (num >= 30)
				{
					int num6 = ((Char.myCharz().cx <= Char.myCharz().mobFocus.getX()) ? (-num4) : num4);
					Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().cx + num6, Char.myCharz().cy);
					Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
					GameCanvas.clearKeyHold();
					GameCanvas.clearKeyPressed();
					return false;
				...
				if (Char.myCharz().cx > Char.myCharz().mobFocus.getX())
				{
					Char.myCharz().cx = Char.myCharz().mobFocus.getX() + num4 + (flag2 ? 30 : 0);
					Char.myCharz().cdir = -1;
				}
				else
				{
	...
	private void autoPlay()
	{
		...
		if (GameCanvas.isTouch)
		{
			for (int l = 0; l < onScreenSkill.Length; l++)
			{
				...
				int num = ((onScreenSkill[l].template.manaUseType == 2) ? 1 : ((onScreenSkill[l].template.manaUseType == 1) ? (onScreenSkill[l].manaUse * Char.myCharz().cMPFull / 100) : onScreenSkill[l].manaUse));
		...
		for (int m = 0; m < keySkill.Length; m++)
		{
			...
			int num2 = ((keySkill[m].template.manaUseType == 2) ? 1 : ((keySkill[m].template.manaUseType == 1) ? (keySkill[m].manaUse * Char.myCharz().cMPFull / 100) : keySkill[m].manaUse));
	...
	public override void paint(mGraphics g)
	{
		...
		if (!isPaintOther)
		{
            ...
            if (ModGame.isShowChar)
			{
				...
				mFont.tahoma_7_white.drawString(g, "TN: " + NinjaUtil.getMoneys(Char.myCharz().cTiemNang), 20, GameCanvas.h - 160, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, string.Concat(new object[4]
				{
					"HP: ",
				...
				}), 20, GameCanvas.h - 150, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, string.Concat(new object[4]
				{
					"KI: ",
				...
				}), 20, GameCanvas.h - 140, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, "SD: " + NinjaUtil.getMoneys(Char.myCharz().cDamFull) + " - CM: " + Char.myCharz().cCriticalFull + " - Giáp: " + Char.myCharz().cDefull, 20, GameCanvas.h - 130, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, "Vàng: " + NinjaUtil.getMoneys(Char.myCharz().xu) , 20, GameCanvas.h - 120, mFont.LEFT, mFont.tahoma_7b_dark);
                mFont.tahoma_7_white.drawString(g, "Ngọc: " + Char.myCharz().luong + " - Ngọc hồng: " + Char.myCharz().luongKhoa, 20, GameCanvas.h - 110, mFont.LEFT, mFont.tahoma_7b_dark);

            }
            if (ModGame.isShowPet)
			{
				mFont.tahoma_7_white.drawString(g, "Thông tin đệ tử:", (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 180, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, "SM: " + NinjaUtil.getMoneys(Char.myPetz().cPower), (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 170, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, "TN: " + NinjaUtil.getMoneys(Char.myPetz().cTiemNang), (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 160, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, string.Concat(new object[4]
				{
					"HP: ",
				...
				}), (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 150, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, string.Concat(new object[4]
				{
					"KI: ",
				...
				}), (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 140, mFont.LEFT, mFont.tahoma_7b_dark);
				mFont.tahoma_7_white.drawString(g, "SD: " + NinjaUtil.getMoneys(Char.myPetz().cDamFull) + " - CM: " + Char.myPetz().cCriticalFull + " - Giáp: " + Char.myPetz().cDefull, (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 130, mFont.LEFT, mFont.tahoma_7b_dark);
                int cStamina = (int)Char.myPetz().cStamina;
                int cMaxStamina = (int)Char.myPetz().cMaxStamina;
                mFont.tahoma_7_white.drawString(g, "Thể lực: " + (Char.myPetz().cStamina * 100 / Char.myPetz().cMaxStamina) + "% " , (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 120, mFont.LEFT, mFont.tahoma_7b_dark);
                mFont.tahoma_7_white.drawString(g, "Trạng thái: " + ModGame.petStatus[Char.myPetz().petStatus], (ModGame.isShowChar ? 150 : 20), GameCanvas.h - 110, mFont.LEFT, mFont.tahoma_7b_dark);

            }

            if (ModGame.isSanBoss)
			{
				g.setColor(Color.red);
				for (int num15 = 0; num15 < vCharInMap.size(); num15++)
				{
					Char char6 = (Char)vCharInMap.elementAt(num15);
					if (char6.cTypePk == 5)
					{
				...
				try
				{
					...
					foreach (string item in listBoss.AsEnumerable().Reverse())
					{
						...
						int num17 = (int)timeSpan.TotalSeconds;
						mFont mFont2 = ((!array[1].Trim().Contains(TileMap.mapName)) ? mFont.tahoma_7_white : mFont.tahoma_7_red);
						mFont2.drawString(g, array[0].Replace("BOSS ", "") + array[1].Replace("zona", "khu") + " - " + ((num17 < 60) ? (num17 + "giây") : (timeSpan.Minutes + "phút")) + " trước", 500, GameCanvas.h - num16, mFont.RIGHT);
						num16 -= 10;
					}
				}
				catch
			...
			if ((TileMap.mapID == 128 || TileMap.mapID == 127) && mabuPercent != 0)
			{
				...
				g.setColor(16711680);
				g.setClip(num18 - 25, num19 - 110, mabuPercent, 4);
			...
			for (int num20 = 0; num20 < vCharInMap.size(); num20++)
			{
				Char char7 = null;
				try
				{
					char7 = (Char)vCharInMap.elementAt(num20);
				}
				catch (Exception)
				{
				}
				if (char7 != null && char7.isFusion && Char.isCharInScreen(char7))
				{
					char7.tFusion++;
					if (GameCanvas.gameTick % 3 == 0)
					{
					...
					if (char7.tFusion >= 100)
					{
						char7.fusionComplete();
					}
				}
			}
			...
			resetTranslate(g);
			int num21 = 100 + ((Char.vItemTime.size() != 0) ? (textTime.size() * 12) : 0);
			if (Char.myCharz().clan != null)
			{
				int num22 = 0;
				int num23 = 0;
				int num24 = (GameCanvas.h - 100 - 60) / 12;
				for (int num25 = 0; num25 < vCharInMap.size(); num25++)
				{
					Char char8 = (Char)vCharInMap.elementAt(num25);
					if (char8.clanID == -1 || char8.clanID != Char.myCharz().clan.ID)
					{
						continue;
					}
					if (char8.isOutX() && char8.cx < Char.myCharz().cx)
					{
						int num26 = num24;
						...
						if (num22 <= num26)
						{
							mFont.tahoma_7_green.drawString(g, char8.cName, 20, num21 - 12 + num22 * 12, mFont.LEFT, mFont.tahoma_7_grey);
							char8.paintHp(g, 10, num21 + num22 * 12 - 5);
							num22++;
						}
					}
					else if (char8.isOutX() && char8.cx > Char.myCharz().cx && num23 <= num24)
					{
						mFont.tahoma_7_green.drawString(g, char8.cName, GameCanvas.w - 25, num21 - 12 + num23 * 12, mFont.RIGHT, mFont.tahoma_7_grey);
						char8.paintHp(g, GameCanvas.w - 15, num21 + num23 * 12 - 5);
						num23++;
					}
				}
			}
			...
			for (int num27 = 0; num27 < Char.vItemTime.size(); num27++)
			{
				((ItemTime)Char.vItemTime.elementAt(num27)).paint(g, cmdMenu.x + 32 + num27 * 24, 55);
			}
			for (int num28 = 0; num28 < textTime.size(); num28++)
			{
				((ItemTime)textTime.elementAt(num28)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 45 : 90) + num28 * 12);
			}
			paintXoSo(g);
			...
			{
				for (int num29 = 0; num29 < strPaint.Length; num29++)
				{
					mFont.tahoma_7b_white.drawString(g, strPaint[num29], 5, 85 + num29 * 18, 0, mFont.tahoma_7b_dark);
				}
			}
		}
		int num30 = 0;
		int num31 = GameCanvas.hw;
		if (num31 > 200)
		{
			num31 = 200;
		}
		paintPhuBanBar(g, num30 + GameCanvas.w / 2, 0, num31);
			...
			else if (Char.myCharz().statusMe != 14)
			{
				...
				else if (!gamePad.isSmallGamePad)
				{
					...
					else
					{
						...
						g.setColor(16777215);
						g.fillRect(xHP + 10, yHP + 10 + ((num2 != 0) ? (20 - num2) : 0), 20, (num2 == 0) ? 18 : num2);
			...
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num3; i++)
				{
					if (Main.isPC)
					{
						...
						if (TField.isQwerty)
						{
							string[] obj = new string[10] { "1", "2", "3", "4", "5", "6", "7", "8", "9", null };
							array2 = obj;
							obj[9] = "0";
						}
						else
						{
							string[] obj2 = new string[5] { "7", "8", "9", "10", null };
							array2 = obj2;
							obj2[4] = "11";
						}
	...
	public void actionPerform(int idAction, object p)
	{
		...
		case 11059:
		{
			Skill skill3 = onScreenSkill[selectedIndexSkill];
			doUseSkill(skill3, false);
			center = null;
			break;
		}
		case 2:
	...
	public void chatVip(string chatVip)
	{
		...
		if (ModGame.isSanBoss && chatVip.StartsWith("BOSS"))
		{
			if (listBoss.Count > 5)
			{
				listBoss.RemoveAt(0);
			}
			listBoss.Add(chatVip.Replace(" vừa xuất hiện tại", " ->>") + "-" + DateTime.Now.ToString("HH:mm:ss"));
		}
	...
	private void autoTanSat()
	{
		...
		if (GameCanvas.isTouch)
		{
			for (int l = 0; l < onScreenSkill.Length; l++)
			{
				...
				int num9 = ((onScreenSkill[l].template.manaUseType == 2) ? 1 : ((onScreenSkill[l].template.manaUseType == 1) ? (onScreenSkill[l].manaUse * Char.myCharz().cMPFull / 100) : onScreenSkill[l].manaUse));
				if (Char.myCharz().cMP >= num9)
				{
					if (skill == null)
					{
						skill = onScreenSkill[l];
		...
		for (int m = 0; m < keySkill.Length; m++)
		{
			...
			int num10 = ((keySkill[m].template.manaUseType == 2) ? 1 : ((keySkill[m].template.manaUseType == 1) ? (keySkill[m].manaUse * Char.myCharz().cMPFull / 100) : keySkill[m].manaUse));
			if (Char.myCharz().cMP >= num10)
			{
				if (skill == null)
				{
					skill = keySkill[m];

...
public class Panel : IActionListener, IChatable
{
	...
	private void paintPetInfo(mGraphics g)
	{
		...
		if (Char.myPetz().cMaxStamina > 0)
		{
            ...
            int num = Char.myPetz().cStamina * 100 / Char.myPetz().cMaxStamina;
            mFont.tahoma_7_yellow.drawString(g, mResources.vitality + " : " + num + "%", X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
        }
		g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
	}
	...
	private void paintCharInfo(mGraphics g, Char c)
	{
		...
		if (c.cMaxStamina > 0)
		{
            ...
            int num = c.cStamina * 100 / c.cMaxStamina;
            mFont.tahoma_7_yellow.drawString(g, mResources.vitality + " : " + num + "%", X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
        }
		g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
		if (c.cPower > 0)
		{
	...
	private void doFireSkill()
	{
		...
		if (selected == 0 || selected == 1 || selected == 2 || selected == 3 || selected == 4 || selected == 5)
		{
			...
			if (selected == 1)
			{
				...
				{
					GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Char.myCharz().cTiemNang + mResources.not_enough_potential_point2 + (Char.myCharz().cMPGoc + num2), false);
					return;
				}
			...
			if (selected == 2)
			{
				...
				{
					GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + Char.myCharz().cTiemNang + mResources.not_enough_potential_point2 + cDamGoc * 100, false);
					return;
				}
			...
			if (selected == 3)
			{
				...
				{
					GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + NinjaUtil.getMoneys(Char.myCharz().cTiemNang) + mResources.not_enough_potential_point2 + NinjaUtil.getMoneys(50000 + Char.myCharz().cDefGoc * 1000), false);
					return;
				}
			...
			else if (selected == 4)
			{
				...
				{
					GameCanvas.startOKDlg(mResources.not_enough_potential_point1 + NinjaUtil.getMoneys(Char.myCharz().cTiemNang) + mResources.not_enough_potential_point2 + NinjaUtil.getMoneys(num4), false);
					return;
				}
